...
Ollama serves its local API at `http://localhost:11434/api` by default. `vibelang` uses the non-streaming `POST /api/chat` endpoint with structured JSON output, falls back to `POST /api/generate` when needed, and now sends tighter per-helper JSON schemas so models see exact helper argument contracts instead of one loose `arguments` object.
